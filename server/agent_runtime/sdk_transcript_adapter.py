...
(filesystem) when ``SCENEWEAVE_SDK_SESSION_STORE=off`` is set.
...
class SdkTranscriptAdapter:
    ...
    (``SCENEWEAVE_SDK_SESSION_STORE=off``) still works.
    ...
    async def _read_via_legacy(self, sdk_session_id: str) -> list[dict[str, Any]]:
        """Filesystem fallback for SCENEWEAVE_SDK_SESSION_STORE=off."""
