...
ARCHIVE_MANIFEST_NAME = "sceneweave-export.json"
...
class ProjectArchiveService:
        ...
        *,
        ...
        try:
            with zipfile.ZipFile(archive_path) as archive:
                ...
                with tempfile.TemporaryDirectory(prefix="sceneweave-import-") as temp_dir:
    ...
    def _prepare_export_snapshot(
        ...
        *,
        ...
        temp_dir = tempfile.TemporaryDirectory(prefix="sceneweave-export-")
    ...
    def _locate_project_root(
        ...
        manifest_members = [
            member for member in visible_members if member.parts[-1] == ARCHIVE_MANIFEST_NAME
        ]
        if manifest_members:
            ...
            if len(root_candidates) != 1:
                raise ProjectArchiveValidationError(
                    "导入包校验失败",
                    errors=["ZIP 中包含多个 sceneweave-export.json，无法确定项目根目录"],
                )
