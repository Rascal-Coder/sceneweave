...
_ENV_VAR = "SCENEWEAVE_SDK_SESSION_STORE"
_VALID_MODES = frozenset({"db", "off", ""})

logger = logging.getLogger("sceneweave.session_store.config")

FlushMode = Literal["eager", "batched"]

_FLUSH_ENV_VAR = "SCENEWEAVE_SDK_SESSION_STORE_FLUSH"
...
def session_store_enabled() -> bool:
    """True when SCENEWEAVE_SDK_SESSION_STORE is anything but 'off' (case-insensitive).
...
def session_store_flush_mode() -> FlushMode:
    ...
    SCENEWEAVE_SDK_SESSION_STORE_FLUSH=batched for the legacy end-of-turn
