...
const STEPS = [
  ...
  },
  {
    ...
    desc: "OpenClaw 会自动读取 SceneWeave Skill 文档，获取所有可用工具与 API 的使用方式",
  },
  {
    step: "03",
    title: "OpenClaw 与 SceneWeave 交互并创建视频",
    desc: "描述你的创作需求，OpenClaw 将调用 SceneWeave 完成项目管理、剧本生成和视频创作",
  },
] as const;
...
export function OpenClawModal({ onClose }: OpenClawModalProps) {
  ...
  const systemPrompt = useMemo(
    () => `学习 ${skillUrl} 然后遵循 skill，了解如何使用 SceneWeave 创作视频`,
    [skillUrl],
  );
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-800 bg-gray-900 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">将 SceneWeave 接入 OpenClaw AI Agent</p>
            </div>
          </div>
          <button
            type="button"
