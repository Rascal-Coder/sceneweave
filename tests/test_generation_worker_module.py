...
class TestReadIntEnv:
    def test_default_when_unset(self, monkeypatch):
        monkeypatch.delenv("SCENEWEAVE_INT", raising=False)
        assert _read_int_env("SCENEWEAVE_INT", 3, minimum=1) == 3

    def test_default_when_bad(self, monkeypatch):
        monkeypatch.setenv("SCENEWEAVE_INT", "bad")
        assert _read_int_env("SCENEWEAVE_INT", 3, minimum=1) == 3

    def test_minimum_enforced(self, monkeypatch):
        monkeypatch.setenv("SCENEWEAVE_INT", "0")
        assert _read_int_env("SCENEWEAVE_INT", 3, minimum=2) == 2
